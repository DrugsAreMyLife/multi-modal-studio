...
export function SelectionChatPopup({ id, context, initialPosition }: SelectionChatPopupProps) {
    ...
    const { messages, sendMessage: append, isLoading } = useChat({
        id: `detached-${id}`,
        initialMessages: [
            { id: 'context', role: 'system', content: `The user has highlighted the following text as context: "${context}". Please provide concise answers based on this context.` }
        ]
    });
    ...
    const handleDropInChat = () => {
        ...
        messages.filter(m => m.id !== 'context').forEach(m => {
            addMessage({
                role: m.role as any,
    ...
    return (
        <motion.div
            ...
            className="fixed z-[200] w-80 h-96 shadow-2xl pointer-events-auto"
        >
            <Card className="flex flex-col h-full border-primary/20 bg-background/95 backdrop-blur-xl overflow-hidden ring-1 ring-primary/10">
                {/* Header */}
                <div className="flex items-center justify-between p-2 border-b border-border bg-muted/50 cursor-move group">
                    <div className="flex items-center gap-2">
                        <GripVertical size={14} className="text-muted-foreground opacity-50 group-hover:opacity-100" />
                        <span className="text-xs font-semibold text-primary uppercase tracking-wider">Ask AI</span>
                    </div>
                ...
                <ScrollArea className="flex-1 p-3">
                    <div ref={scrollRef} className="space-y-4">
                        {messages.filter(m => m.id !== 'context').map((m, i) => (
                            <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[85%] p-2 rounded-lg text-xs leading-relaxed ${m.role === 'user'
                ...
                <div className="p-2 border-t border-border bg-muted/30">
                    <form onSubmit={handleSend} className="flex gap-2">
                        <Input
                            ...
                            className="h-8 text-xs bg-background border-none ring-1 ring-border focus-visible:ring-primary shadow-none"
                        />
                        <Button type="submit" size="icon" className="h-8 w-8 rounded-md shrink-0" disabled={isLoading || !input.trim()}>
                            <Send size={14} />
                        </Button>
                    </form>
                </div>
