  ...
  return (
    <>
      ...
      {selection && (
        <div
          ref={tooltipRef}
          className="animate-in fade-in zoom-in fixed z-[300] mb-2 -translate-x-1/2 -translate-y-full duration-200"
          style={{ left: selection.x, top: selection.y }}
        >
          <Button
            size="sm"
            onClick={handleAskAI}
            className="bg-primary hover:bg-primary/90 text-primary-foreground border-primary-foreground/20 flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-semibold shadow-lg"
          >
      ...
      {/* Active Popups */}
      {popups.map((p) => (
        <SelectionChatPopup key={p.id} id={p.id} context={p.context} initialPosition={p.position} />
      ))}
    </>
  );
}
