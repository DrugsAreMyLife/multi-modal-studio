...
export function IconModelSelector() {
  ...
  const activeModel =
    AVAILABLE_ICON_MODELS.find((m) => m.id === selectedModelId) || AVAILABLE_ICON_MODELS[0];
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          className="border-border/50 bg-background/20 hover:bg-background/40 h-9 gap-2 border px-3"
        >
