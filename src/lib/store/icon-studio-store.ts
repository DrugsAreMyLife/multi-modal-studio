...
import { StyleDNA, IconConcept, IconAsset, GenerationPass, IconStudioTab, IconGenerationSettings } from '@/lib/types/icon-system';
...
export const useIconStudioStore = create<IconStudioStore>()(
    persist(
        (set, get) => ({
            ...
            updateStyleDNA: (updates) => set(state => ({
                styleDNA: { ...state.styleDNA, ...updates }
            })),

            updateGenerationSettings: (updates) => set(state => ({
