...
interface DawStore extends ProjectState {
    ...
    addClip: (trackId: string, audioClipId: string, name: string, duration: number, startTime: number) => void;
...
export const useDawStore = create<DawStore>()(
    persist(
        (set, get) => ({
            tracks: [
                { id: 't1', name: 'Vocals', type: 'audio', volume: 0.8, pan: 0, muted: false, soloed: false, color: '#ec4899' },
                { id: 't2', name: 'Drums', type: 'audio', volume: 0.7, pan: 0, muted: false, soloed: false, color: '#3b82f6' },
                { id: 't3', name: 'Synth', type: 'audio', volume: 0.6, pan: 0, muted: false, soloed: false, color: '#a855f7' },
                { id: 't4', name: 'Bass', type: 'audio', volume: 0.75, pan: 0, muted: false, soloed: false, color: '#eab308' },
            ],
            clips: {},
            isPlaying: false,
            currentTime: 0,
            ...
            removeTrack: (id) => set(state => ({
                ...
                clips: Object.fromEntries(Object.entries(state.clips).filter(([_, c]) => c.trackId !== id))
            })),
            ...
            moveClip: (clipId, startTime, trackId) => set(state => {
